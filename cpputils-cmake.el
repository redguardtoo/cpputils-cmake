  ...
  (let (tks
    ...
    (dolist (tk tks v)
      (if next-tk-is-included-dir ;; only for "-isystem"
        ...
        (let ((tkl (length tk)))
          (cond
           ((and (>= tkl 3) (string= (substring tk 0 3) "-I\""))
            (setq v (concat v " -I" (substring tk 3 (- tkl 1)) )))
           ((and (>= tkl 2) (string= (substring tk 0 2) "-I"))
            (setq v (concat v " -I\"" (substring tk 2 tkl) "\"")))
           ((and (>= tkl 2) (string= (substring tk 0 2) "I\""))
            (setq v (concat v " -I" (substring tk 1 tkl) )))
           ((and (>= tkl 1) (string= (substring tk 0 1) "I"))
            (setq v (concat v " -I\"" (substring tk 1 tkl) "\"")))
...
(defun cppcm-reload-all ()
  ...
  (when cppcm-include-dirs
    ...
    (setq company-clang-arguments (append cppcm-include-dirs
