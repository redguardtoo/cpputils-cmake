...
*WARNING*:
cpputils-cmake assumes that in CMakeLists.txt contains a rule to create target/executable which can be process by regular expression.

The rule is either "add_executable" or "add_library". You can use "include" some common.cmake in CMakeLists.txt. But don't put "add_executable" or "add_library" into common.cmake because cpputils-cmake won't read any included *.cmake.
