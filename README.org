* cpputils-cmake.el (current version 0.0.3)
...
* Handy tools
The command `cppcm-get-exe-path-current-buffer` copy the executable's full path into clipboard.

It return the full path at the same time.

You can use it with the command `gud-gdb`:
#+BEGIN_SRC elisp
...
The purpose of above script is to avoid typing the executable's path when you start gdb.
* Contact me
